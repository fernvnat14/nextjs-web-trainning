...
## 🛫 Next.js Check-in App

This project features a Next.js (App Router) migration of the Qoomlee Airline Check-in flutter. It uses responsive, modern designs that map the complete user flow from finding a booking to receiving a boarding pass.
...
1. **Install dependencies** (if not already installed using the `qoomlee/app` directory):
   ```bash
   cd app
   bun install
   ```
